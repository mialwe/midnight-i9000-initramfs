#!/sbin/busybox sh
	...
	/sbin/busybox cp	/system/bin/toolbox \
		/system/bin/sh \
		/system/bin/log \
		/system/bin/linker \
		/sbin/mke2fs \
	...
	/sbin/busybox cp 	/system/lib/liblog.so \
		/system/lib/libc.so \
	...
	/sbin/busybox cp /res/misc/mke2fs.conf /system_in_ram/etc/
	
	/sbin/busybox umount /system
	/sbin/busybox rm -rf /system/bin
	/sbin/busybox ln -s /system_in_ram/* /system
