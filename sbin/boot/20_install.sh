...
if /sbin/busybox [ -f /system/etc/$CONFFILE ];then
    echo "/system/etc/$CONFFILE found, skipping..."
else
    echo "Creating /system/etc/$CONFFILE..."
    /sbin/busybox touch /system/etc/$CONFFILE
fi
...
CONFFILE="midnight_misc.conf"
if /sbin/busybox [ -f /system/etc/$CONFFILE ];then
    # check autoroot option
    if /sbin/busybox [ "`grep NOAUTOROOT /system/etc/$CONFFILE`" ]; then
        echo "AutoROOTing disabled, nothing to do."
    else
        echo "AutoROOTing enabled, let's go..."
